...
import { FaCircleInfo, FaCircleCheck, FaCircleXmark, FaXmark } from "react-icons/fa6";
...
export default function Authenticated({ user, header, children }: PropsWithChildren<{ user: User, header?: ReactNode }>) {
    ...
    const ShowAlert = () => {
        ...
        if (flash.success || flash.error || flash.message || flash.status ) {

            let variant : "default" | "danger" | "success" | "info";
            ...
            if (variant && message) {
                return isShow && <Alert className={"mb-4"} variant={variant}>
                    {icon}
                    <div className={"flex justify-between items-center"}>
                        <AlertDescription>{message}</AlertDescription>
                        <div>
                            <FaXmark className={"cursor-pointer h-6 w-6"} onClick={() => {setIsShow(false)}} />
                        </div>
                    </div>
                </Alert>
            }
                        ...
                        <div className="flex">
                            ...
                            <div className="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                {user.permissions.includes('view_team') &&
                                    <NavLink href={route('teams.my-team')} active={route().current('teams.*')}>
                                        My Team
                                    </NavLink>
                                }
